...
# Fetch a single day's articles
def fetch_day_articles(date_str):
    articles = []
    first = True
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                parsed_articles = parse_articles(response.text, date_str)
                if not parsed_articles:  # Break if no articles are found on this page
                    break
...
# Multithreaded fetching for all dates
def fetch_all_news_multithreaded(start_date):
    all_articles = []
    ...
    while current_date <= end_date:
        date_strings.append(current_date.strftime("%Y_%m_%d"))
        current_date += timedelta(days=1)

    # Use ThreadPoolExecutor for multithreading
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_date = {executor.submit(fetch_day_articles, date_str): date_str for date_str in date_strings}
        for future in as_completed(future_to_date):
            ...
            try:
                ...
                if articles:
                    ...
                    print(f"Successfully fetched articles for {date_str}.")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    fetch_all_news_multithreaded(start_date)